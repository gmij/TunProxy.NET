...
public class IPPacketTests
{
    ...
    public void Parse_ValidIPv4Packet_ReturnsPacket()
    {
        ...
        {
            // IPv4 头部（20 字节）
            0x45, 0x00, 0x00, 28, // 版本 4, IHL 5, TOS, 总长度 28
            ...
            // TCP 头部（20 字节）- 小端字节序（BitConverter 期望）
            0x39, 0x30, // 源端口：12345 (0x3039 小端 = 0x39, 0x30)
            0x50, 0x00, // 目标端口：80 (0x0050 小端 = 0x50, 0x00)
    ...
    public void Parse_UDPPacket_ReturnsPacketWithUDPHeader()
    {
        var packetData = new byte[]
        {
            ...
            // UDP 头部（8 字节）- 小端字节序
            0x35, 0x00, // 源端口：53 (DNS) 小端
            0x35, 0x00, // 目标端口：53 小端
            0x08, 0x00, // 长度小端
            0x00, 0x00, // 校验和
