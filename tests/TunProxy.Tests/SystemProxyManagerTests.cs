...
public class SystemProxyManagerTests
{
    ...
    [Fact]
    public void SetPacUrl_PersistsOriginalSystemProxyBackupBeforeOverwriting()
    {
        if (!OperatingSystem.IsWindows())
            return;
        ...
        try
        {
            ...
            var store = new SystemProxyBackupStore(config, configPath);
            var manager = new SystemProxyManager(Registry.CurrentUser, keyPath, store);
            manager.SetPacUrl("http://127.0.0.1:50000/proxy.pac");

            using var pacKey = Registry.CurrentUser.OpenSubKey(keyPath, writable: false)!;
            Assert.Equal(0, (int)(pacKey.GetValue("ProxyEnable") ?? -1));
            Assert.Equal("old:80", pacKey.GetValue("ProxyServer") as string);
            Assert.Equal("<local>", pacKey.GetValue("ProxyOverride") as string);
            Assert.Equal("http://127.0.0.1:50000/proxy.pac", pacKey.GetValue("AutoConfigURL") as string);
    ...
    [Fact]
    public void ClearPacUrl_RestoresPersistedBackupAndClearsIt()
    {
        if (!OperatingSystem.IsWindows())
            return;
        ...
        try
        {
            ...
            using (var key = Registry.CurrentUser.CreateSubKey(keyPath, writable: true)!)
            {
                ...
                key.SetValue("ProxyServer", "old:80", RegistryValueKind.String);
                key.SetValue("ProxyOverride", "<local>", RegistryValueKind.String);
                key.SetValue("AutoConfigURL", "http://127.0.0.1:50000/proxy.pac", RegistryValueKind.String);
            }
