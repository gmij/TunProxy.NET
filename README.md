...
TunProxy.NET 是一个基于 .NET 8 的代理运行时，面向需要“能看见、能配置、能诊断”的本地代理与 TUN 透明代理场景。它可以把系统或应用流量转发到上游 SOCKS5/HTTP 代理，并结合 GFWList、GeoIP、FakeIP DNS、手动域名规则和直连失败回退策略做路由决策。

项目当前提供三类入口：

- Web 控制台：静态 Vue global + Ant Design 页面，围绕 Status、Config、Rules、DNS、Logs 五个页面组织运行时观测与配置工作流。
- CLI：适合 Linux、macOS 或无桌面环境，可初始化配置、检查资源并后台运行服务。
- Windows 托盘：负责打开控制台、启动/停止运行时、安装/卸载 `TunProxyService`，以及协调保存配置后的重启。

当前 Web 控制台采用新的侧边导航与运维工作台布局：

- Status：汇总运行状态、连接健康、流量趋势和 TUN 诊断。
- Config：按上游代理、路由资源、系统代理模式的顺序完成配置。
- Rules：集中维护手动代理域名、直连域名，以及直连失败自动切代理阈值。
- DNS：在 TUN 模式下查看解析记录、路由原因、缓存状态和清理动作。
- Logs：查看实时内存日志，按连接、DNS、警告、错误快速筛选。
...
- 运维式 Web 控制台：统一侧边导航、语言切换、摘要卡片、右侧信息面板和响应式布局，适合长期挂着观察状态。
...
4. 如需固定代理域名、直连域名或调整直连失败自动切代理策略，到 Rules 页补充路由规则。
5. 返回 Config 页选择系统代理模式：PAC、Global、TUN 或 None。
6. 保存并重启。TUN 模式在 Windows 上建议由托盘安装服务后运行。
...
状态页现在以运维看板方式组织信息：顶部状态带汇总运行状态、模式标签、代理摘要和服务操作；中部展示实时流量卡片与滚动趋势；右侧集中展示 TUN 诊断和最近连接异常。页面默认每 5 秒刷新，并保留滚动流量样本用于绘制发送/接收变化。
...
右侧面板展示配置摘要、PAC 地址与快捷操作。保存配置会写入 `tunproxy.json`，再通过 `tunproxy.restart` 交给托盘或服务协调重启。

### Rules

Rules 页把手动域名规则从基础配置中拆分出来，专门用于维护：

- `proxyDomains`：始终走代理的域名。
- `directDomains`：始终直连的域名。
- 直连失败自动切代理：失败阈值、统计窗口和临时代理 TTL。

这让日常配置和路由策略调整分层更清晰，也更适合在运行中反复微调。

### DNS

DNS 页只在 TUN 模式下显示完整数据。本地代理模式下，页面会明确提示 DNS 拦截不可用；TUN 模式下则提供顶部摘要卡、记录列表、路由图例与侧边说明。
...
日志页每 2 秒拉取内存日志，最多保留 1000 行，默认过滤连接日志。页面采用终端视图加侧边摘要的结构，支持：
...
配置文件为程序目录下的 `tunproxy.json`。Web 控制台中的 Config 页和 Rules 页，以及 CLI 的 `config` 命令，都会更新此文件。
...
智能路由与 Rules 页中的手动域名规则共同生效，大致按以下顺序执行：
...
说明：Rules 页没有单独的 API，它和 Config 页一样通过 `GET/POST /api/config` 读写同一份配置，只是界面上分成两个工作流页面。
...
上游代理、TUN 适配器、DNS 服务、路由和监听端口都是运行时状态。Config 页或 Rules 页中的保存操作都只负责写入 `tunproxy.json`，真正生效需要重启。Web 控制台写入 `tunproxy.restart`，托盘或服务在旧运行时停止后再启动新进程，避免端口、路由和驱动占用竞争。

### 为什么 TUN 没有直接启动？

如果启用了 GeoIP 或 GFWList，程序会检查对应文件是否真的有效。文件存在但无法读取、无法解析时仍会阻止直接进入 TUN，并保留本地代理设置模式。请先在 Config 页准备资源，再结合需要在 Rules 页调整手动规则，最后保存并重启。
